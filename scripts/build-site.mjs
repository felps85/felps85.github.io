  ...
  return `
    <script>
      (() => {
        ...
        const queueStorageKey = "__fs_analytics_queue__:" + namespace;
        let flushTimer = null;
        ...
        function writeQueue(queue) {
          try {
            const entries = Object.entries(queue).filter(([, count]) => Number(count) > 0);
            if (!entries.length) {
              window.localStorage.removeItem(queueStorageKey);
              return;
            }

            window.localStorage.setItem(queueStorageKey, JSON.stringify(Object.fromEntries(entries)));
          } catch {}
        }

        function enqueueCounter(name, count = 1) {
          const queue = readQueue();
          queue[name] = Number(queue[name] || 0) + count;
          writeQueue(queue);
        }

        function scheduleQueueFlush(delay = 1500) {
        ...
        function sendCounter(name) {
          ...
          return fetch(url, { method: "GET", keepalive: true, cache: "no-store" }).then((response) => {
            if (!response.ok) throw new Error("Bad response");
          });
        }

        async function flushQueuedCounters() {
          ...
          try {
            ...
            for (const [name, count] of entries) {
              ...
              if (remaining > 0) {
                nextQueue[name] = remaining;
              } else {
                delete nextQueue[name];
              }
            }
        ...
        window.addEventListener("online", () => flushQueuedCounters());
      ...
      function loadDashboard() {
        ...
        Promise.all(
          dashboard.pages.map(async (page) => {
            const count = await fetchCount(dashboard.namespace, page.route);
            const countFailed = count === null;
            const safeCount = countFailed ? 0 : count;
            const activeBuckets = safeCount
              ? await fetchActiveBuckets(dashboard.namespace, page.route)
              : 0;
            const activeFailed = safeCount ? activeBuckets === null : false;

            return {
              ...page,
              count: safeCount,
              activeBuckets: activeFailed ? 0 : activeBuckets,
              hasError: countFailed || activeFailed
            };
          })
        )
          .then((results) => {
            ...
            const failedRows = sorted.filter((item) => item.hasError).length;
            ...
            searchSuggestionsEl.innerHTML = sorted
              ...
              .join("");
            errorEl.hidden = failedRows === 0;
            errorEl.textContent =
              failedRows === 0
                ? "Analytics could not be loaded right now."
                : failedRows === 1
                  ? "1 row could not be refreshed and is showing 0 for now."
                  : String(failedRows) + " rows could not be refreshed and are showing 0 for now.";
            syncSortUi();
            updateSearchResults();
          })
          .catch(() => {
